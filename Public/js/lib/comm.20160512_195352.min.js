...
var _login= {
    ...
    _hide: function() {
        _tc._hide("login");
        $('#cover2').css({"z-index":1000});
        $('#login').css({"z-index":3001});
        if($("#dt_review_box").css("display")=="block"){
            _cover._show("cover2");
            $("#cover2").bind("click",function(){
                _cover._hide('cover2');
                $("#dt_review_box").hide();
            })
        }else if($("#join_box").css("display")=="block"){
            _cover._show("cover2");
            $("#cover2").bind("click",function(){
                _cover._hide('cover2');
                _joinBox._hide();
...
var _user= {
    ...
    _init: function() {
        ...
        if (_user._login()) {
        	$(".mem_r,.a_topLogin_out").show();
            $(".mem_r2,.a_topLogin").hide();
...
var _loadingBottom= {
    _init: function(text, fun, id) {
        $("#" + id).html("<a>" + text + "<img src=\"http://cdn.hudongba.com/images3/loading_bottom.png\"></a>");
        $("#" + id).bind("click", fun);
        $("#" + id).show();
    },
    _loading: function(id) {
        $("#" + id).html("<p class='jz_More'><img src='http://cdn.hudongba.com/static_v4/images/other/loading2.gif'><span>加载中…</span></p>");
        $("#" + id).unbind("click");
        $("#" + id).show();
    },
    _hide: function(id) {
...
var _login4={
   ...
   _ok:function(json,code){
	  ...
	  if(way==1){
		  ...
		  _partyPostPC._postFront(_t._get("login_Uid"),_t._get("login_Cha"));//发布活动
	  }else if(way==6){
...
var _share_hdb = {
    _shareqzone: function () {
        var p = {
            ...
            site: _domain,
            pics: dataForShare.share_big_img == "" ? dataForShare.qq_icon : "http://cdn.hudongba.com" + dataForShare.share_big_img,
            style:'201',
            width:25,
            height:25
        };
    ...
    _sharesina: function () {
        var param = {
            ...
            title: dataForShare.title,
            searchPic: true,
            pic: dataForShare.share_big_img == "" ? dataForShare.weibo_icon : "http://cdn.hudongba.com" + dataForShare.share_big_img,
            language: 'zh_cn'
        };
			    ...
			    if (!_user._login()) {//未登录
			      ...
			      return;
			    	...
			    	if(leanUser_object_id){//如果注册了 id
                        if(_user._inHudongba()){
                            HudongbaJsBridge["callim"](leanUser_object_id, _info._title, dataForShare.share_big_img == "" ? "http://cdn.hudongba.com/images/face_default.png" : dataForShare.share_big_img, dataForShare.weixin_url);
                        }
		        	}else{//未注册id，就给注册id
	        ...
	        if (/MicroMessenger/i.test(navigator.userAgent)) { //这是微信平台下浏览器
	        	...
	        	if(leanUser_object_id){//如果注册了 id
	        		if(_info._type == 'timeline'){
	        			...
	        			var data = encode.base64Encode(leanUser_object_id + "::" + encode._deCode(_info._title) +"::"+ encode._deCode((dataForShare.share_big_img == "" ? "http://cdn.hudongba.com/images/face_default.png" : dataForShare.share_big_img)) + "::" + encode._deCode(dataForShare.weixin_url));
	        		}
...
$(document).ready(function() {
  ...
  //有footer_page_type参数 说明引用了 public <#--21.PC端公共灰色底部&登录弹窗--> 需要初始化底部栏数据：互动吧简介、友情链接、热门城市、友情链接 && 导航栏姓名、头像
    ...
    if(typeof _info != 'undefined'){
        info_id = _info._id || "";
        info_type = _info._type || "";
    }
        
  //bi统计
  var Biurl = _domain + "hdb-bi?url=" + encodeURIComponent(document.URL) + "&t=" + +new Date() + "&refer=" + encodeURIComponent(document.referrer) + "&infoId=" + info_id + "&infoType=" + info_type +"&userId="+_user._id()+"";
  var a = new Image();
  a.src = Biurl;
...
//异步ajax初始化底部栏数据：互动吧简介、友情链接、热门城市、友情链接 && 用户导航栏名称、头像
...
function getFooterInfoOk(obj){
    ...
    //用户名称、头像 - 这里是为了处理页面是静态化页面时 页面没有展示出来登录名  所以需要展示一次
    if(_user._login() && $("#a_top_user").html() == ""){

        $("#a_top_user").html(obj.user_nick);

        if(obj.user_head_url != ""){
            $("#a_top_pic img").attr("src", obj.user_head_url);
        }else if(obj.user_shop_head_url != ""){
            $("#a_top_pic img").attr("src", obj.user_shop_head_url);
        }
    ...
    //友情链接 20160408 友情链接改为非异步加载 shaoguoqing
    //if(obj.linkList){
    //
    //    var html = "";
    //
...
function goBack(){
    ...
    if(refer!="" && refer.split("?")[0]!=window.location.href.split("?")[0]){
        //window.history.go(-1);这种方式会导致报名成功之后点击返回时，页面不刷新 ，报名状态还是我要报名
        location.href = refer;
    }else{
        location.href="/info_discovery";
    }
};
//a标签链接优化
var _a_link = function(that){ 
	var url = that.attr("href");
	var url_arr = url.split('=');
	var url_arr1 = url_arr[0].split('?');
	if(url_arr1[1] != _HDB_POS_KEY){
		var data = that.attr("data-val");
		that.attr({"href":url+"?"+_HDB_POS_KEY+data});
	};
};
