...
//   			alert(area_id);
				...
				avalon.ajax({
					...
					if(page_num==1){
						...
						if(data.area_list!=""){
							...
							index_recommend_model.getAreaName(data.area_list);
						}else{
		...
		getAreaName : function (area_list) {

			if(index_recommend_model.area_id == ''){
				index_recommend_model.area_name = '全国';
				return ;
			}

			area_list.forEach(function(el) {   //遍历数据，取出对象数据
				if(el.area_id36 == index_recommend_model.area_id ){
					index_recommend_model.area_name = el.area_name;
				}
			});

			if(index_recommend_model.area_name == ''){
				index_recommend_model.area_name = "全国";
			}
		}
   	});
